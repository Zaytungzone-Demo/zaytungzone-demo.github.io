...
function resolveAssetUrl(path) {
  ...
  const dir = p.endsWith("/") ? p : (p.includes(".") ? p.substring(0, p.lastIndexOf("/") + 1) : p + "/");
  const base = new URL(dir, window.location.origin).href;
    ...
    .map(
      ...
      <article class="campaign-card">
        <div class="campaign-image ${campaign.imagePosition}">
          <img src="${resolveAssetUrl(campaign.imageUrl)}" alt="${escapeHtml(campaign.title[state.locale])}" ${campaign.order === 1 ? "" : 'loading="lazy"'} />
...
function renderProduct(item) {
  ...
  return `
    <article class="product-card ${item.soldOut ? "sold-out" : ""}">
      ${
        item.imageUrl
          ? `<img src="${resolveAssetUrl(item.imageUrl)}" alt="${name}" loading="lazy" width="180" height="140" />`
...
const pathMatch = location.pathname.match(/\/kampanya\/([\w-]+)$/);
const deepLink = hashMatch || pathMatch;
if (deepLink) openCampaign(deepLink[1]);

window.addEventListener("hashchange", () => {
  const match = (location.hash || "").match(/#kampanya\/([\w-]+)/);
  if (match) openCampaign(match[1]);
  else if ($("modal-root")?.firstElementChild) closeCampaign();
});
